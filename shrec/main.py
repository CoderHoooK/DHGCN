...
def get_parser():
    ...
    parser.add_argument(
        ...
        default = 'config/shrec17/shrec_dgstgcn.yaml',
...
class Processor():
    ...
    def load_model(self):
        ...
        # self.loss = cosCrossEntropyLoss(self.arg.train_feeder_args.label_flag).cuda(output_device)
        if self.arg.weights:
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                end1 = time.time()
                print("train:{}".format(end1-start1))
                start2 = time.time()
                self.eval(epoch, save_score=self.arg.save_score, loader_name=['test'])
                end2 = time.time()
                print("test:{}".format(end2-start2))
            # test the best model
            weights_path = glob.glob(os.path.join(self.arg.work_dir, 'runs-'+str(self.best_acc_epoch)+'*'))[0]
            # weights_path = 'work_dir/shrec17/28jr/runs-68-13872.pt'
