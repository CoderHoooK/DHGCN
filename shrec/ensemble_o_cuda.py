...
if __name__ == "__main__":
    ...
    parser.add_argument('--main-dir',
                        default='shrec/result/14',
                        help='')
    arg = parser.parse_args()
    with open(os.path.join(arg.main_dir, 'label.pkl'), 'rb') as l:
        label = list(pickle.load(l))
    with open(os.path.join('shrec/result/14/j/', 'epoch1_test_score.pkl'), 'rb') as r1:
        r1 = list(pickle.load(r1))
    with open(os.path.join('shrec/result/14/jm/', 'epoch1_test_score.pkl'), 'rb') as r2:
        r2 = list(pickle.load(r2))
    with open(os.path.join('shrec/result/14/jm/', 'epoch1_test_score.pkl'), 'rb') as r3:
        r3 = list(pickle.load(r3))
    with open(os.path.join('shrec/result/14/jm/', 'epoch1_test_score.pkl'), 'rb') as r4:
